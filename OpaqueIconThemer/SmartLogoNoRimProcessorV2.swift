...
/// Strict Smart Logo / Apple Mono base renderer.
///
/// Unlike the historical Smart Logo path, this processor never lets source AA/background pixels
/// participate in the logo material. It creates exactly two materials at 1024px:
/// - background material = inferred original background pulled toward the selected tint;
/// - foreground material = white logo body with an optional selected-colour lower gradient.
///
/// Internal depth is added later by AppleMonoDepthProcessor, which is already restricted to an
/// eroded interior. A final OutlineFreeSmartLogoProcessor pass then re-locks the boundary.
final class SmartLogoNoRimProcessorV2 {
    static let shared = SmartLogoNoRimProcessorV2()
    ...
    func apply(
        ...
        // Smart Logo must not inherit the permissive detector fringe. A slightly stricter cut than
        // the old path removes the coloured/white slivers visible when the logo tint changes.
        ...
        let gradStrength = gradientEnabled ? clamp(gradientStrength, 0, 1) : 0
        ...
        for y in 0..<height {
            ...
            for x in 0..<width {
                ...
                let originalBackground = srgbToLinear(
                    bilinear(corners, x: xNorm, y: yNorm)
                )
                ...
                guard silhouette[i] > 0.5 else {
                    write(linearToSRGB(backgroundMaterial), to: &output, offset: p)
                    continue
                }

                // No source RGB on the logo body at all. That is the key difference from the old
                // Smart Logo tint path: source AA, old background and edge colours cannot survive.
                let foregroundMaterial = mix(
                    white,
                    tintLinear,
                    amount: gradientAmount
                )
                write(linearToSRGB(foregroundMaterial), to: &output, offset: p)
            }
        }
    ...
    private func backgroundCorners(
        pixels: [UInt8],
        width: Int,
        height: Int
    ) -> (RGB, RGB, RGB, RGB) {
